...
test.describe('User testing Suite', () => {
    ...
    test('User logged in successfully', async ({ dashboardPage }) => {
        expect(await dashboardPage.getDashboardTitle()).toBe(DASHBOARD.DASHBOARD);
    });
    ...
    test('Add a person', async ({ dashboardPage }) => {
        ...
        expect(
            await dashboardPage.getPopupMessageText())
                .toBe(DASHBOARD.PERSON_ADDED_MESSAGE);
    });
    ...
    test('Manage people', async ({ dashboardPage }) => {
        ...
        expect(text).toBe(DASHBOARD.PERSON_ADDED_MESSAGE);
